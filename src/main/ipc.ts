...
import { IpcEvents, WEB_CONTENTS_READY_FOR_IPC_SIGNAL, ipcMainEvents } from '../ipc-events';
...
class IpcMainManager extends EventEmitter {
  ...
  constructor() {
    ...
    ipcMain.on(WEB_CONTENTS_READY_FOR_IPC_SIGNAL, (event: Electron.IpcMainEvent) => {
  ...
  public handle(channel: IpcEvents, listener: (event: Electron.IpcMainInvokeEvent, ...args: any[]) => any) {
    // there can be only one, so remove previous one first
    ipcMain.removeHandler(channel);
    ipcMain.handle(channel, listener);
  }

  public handleOnce(channel: IpcEvents, listener: (event: Electron.IpcMainInvokeEvent, ...args: any[]) => any) {
    ipcMain.handleOnce(channel, listener);
  }

  public postMessage(channel: IpcEvents, message: any, transfer?: MessagePortMain[], target?: Electron.WebContents) {
    const _target = target;
    if (!_target) {
      getOrCreateMainWindow().then((window) => {
